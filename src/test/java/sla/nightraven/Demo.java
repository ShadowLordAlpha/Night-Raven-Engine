...
import static org.lwjgl.nanovg.NanoVG.nvgHSLA;
...
public class Demo {
	...
	static class DemoData {
		...
		final ByteBuffer entypo        = loadResource("sla/nightraven/entypo.ttf", 40 * 1024);
		final ByteBuffer RobotoRegular = loadResource("sla/nightraven/Roboto-Regular.ttf", 150 * 1024);
		final ByteBuffer RobotoBold    = loadResource("sla/nightraven/Roboto-Bold.ttf", 150 * 1024);
	...
	private static void drawColorwheel(long vg, float x, float y, float w, float h, float t) {
		...
		// Center triangle
		r = r0 - 6;
		ax = (float)cos(120.0f / 180.0f * NVG_PI) * r;
		ay = (float)sin(120.0f / 180.0f * NVG_PI) * r;
		bx = (float)cos(-120.0f / 180.0f * NVG_PI) * r;
		by = (float)sin(-120.0f / 180.0f * NVG_PI) * r;
		nvgBeginPath(vg);
		nvgMoveTo(vg, r, 0);
		nvgLineTo(vg, ax, ay);
		nvgLineTo(vg, bx, by);
	...
	private static void drawParagraph(long vg, float x, float y, float width, float height, float mx, float my) {
		...
		px = (int)((bounds.get(2) + bounds.get(0)) / 2);
